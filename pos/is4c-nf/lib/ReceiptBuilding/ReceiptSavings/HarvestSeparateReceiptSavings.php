...
*/
class HarvestSeparateReceiptSavings extends DefaultReceiptSavings
{
    public function savingsMessage($trans_num)
    {
        ...
        $msg = '';
        if (($row['transDiscount'] > 0) || ($row['sales'] + $row['memSales'] > 0) || ($row['coupons'] > 0)) {
            $msg = "TODAY YOU SAVED:\n";
        }
        ...
        if ($row['sales'] + $row['memSales'] > 0) {
            $msg .= '  ' . _('Items On Sale = $') . number_format($row['sales'] + $row['memSales'], 2) . "\n";
        }
        if ($row['coupons'] > 0) {
